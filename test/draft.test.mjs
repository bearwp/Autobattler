// Draft state machine tests: snake order, escalating gold, availability,
...
import { Draft, PVP, buildPool, pickOrder, goldForRound } from '../js/pvp/draft.js';
...
test('goldForRound escalates by 50 each round', () => {
  assert.equal(goldForRound(1), 50);
  assert.equal(goldForRound(2), 100);
  assert.equal(goldForRound(3), 150);
});

test('pickOrder produces a correct snake for 4 picks each', () => {
  assert.deepEqual(pickOrder(4), ['a', 'b', 'b', 'a', 'a', 'b', 'b', 'a']);
  assert.equal(pickOrder(4).length, 8);
  assert.equal(pickOrder(3).length, 6);
  assert.deepEqual(pickOrder(1), ['a', 'b']);
});
...
test('draft alternates in snake order and ends roundComplete', () => {
  const d = new Draft(pool(), { picksPerPlayer: 4, baseGold: 200 });
  const ids = ['m1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8'];
  const expectedTurn = ['a', 'b', 'b', 'a', 'a', 'b', 'b', 'a'];

  for (let i = 0; i < ids.length; i++) {
    assert.equal(d.turn, expectedTurn[i], `turn at pick ${i}`);
    assert.ok(d.pick(d.turn, ids[i]), `pick ${i} should succeed`);
  }

  assert.equal(d.roundComplete, true);
  assert.equal(d.phase, 'betweenRounds');
  assert.equal(d.picks.a.length, 4);
  assert.equal(d.picks.b.length, 4);
  assert.deepEqual(d.picks.a, ['m1', 'm4', 'm5', 'm8']);
  assert.deepEqual(d.picks.b, ['m2', 'm3', 'm6', 'm7']);
});
...
test('a player cannot pick a member they cannot afford', () => {
  ...
  p[0].salary = 1000;
  const d = new Draft(p, { picksPerPlayer: 4, baseGold: 100 });
...
test('gold is deducted on pick and cannot go negative', () => {
  ...
  // m2 costs 60 > 40 remaining, so it must be rejected.
  assert.equal(d.pick('b', 'm2'), true); // b has full budget
  assert.equal(d.gold.b, 40);
});

test('nextRound resets picks and escalates gold but keeps the pool', () => {
  const d = new Draft(pool(), { picksPerPlayer: 4, rounds: 3, baseGold: 200 });
  const ids = ['m1', 'm2', 'm3', 'm4', 'm5', 'm6', 'm7', 'm8'];
  for (const id of ids) d.pick(d.turn, id);
  assert.equal(d.phase, 'betweenRounds');
...
test('serialize/deserialize round-trips the draft state', () => {
  ...
  // The guest can continue picking from the restored state.
